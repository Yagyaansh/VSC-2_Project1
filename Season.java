...
public class Season {
	...
	public void offSeason(Player_Pool players, Coach_Pool coachPool) {
		...
		for (Player player : players.getPlayerPool()) {
			...
			if (player.getAge() == 40) {
				...
				if (player.isInATeam()) {
					...
					// then get the team -> get the roster -> remove player from
					// the roster
				...
				// continue; // if the player has been removed - no need to
				// update his fit score
			}
		...
		for (int i = 0; i < teams.size(); i++) {
			...
			for (int j = 0; j < team.getRoster().size(); j++) {
				...
				// player.increaseAge(); // this is wrong ! All the players must
				// age not just those in team rosters !!
				player.updateFit(coach); // this is fine because players not in
											// any team roster dont have a coach
			}
	...
	public void redraftPlayers(MinPQ_Athleticism pqOffensive, MinPQ_Athleticism pqDefensive,
			ArrayList<Team> teamLosstoWinOrder) {
		int rosterSize = 0;
		int offensiveSize = 0;
		int defensiveSize = 0;

		Iterator<Team> it = teamLosstoWinOrder.iterator();
		while (it.hasNext()) { // all teams do not have 50 players we keep
								// drafting
			if (it.next().getRoster().size() != 50) {

				for (Team t : teamLosstoWinOrder) {
					rosterSize = t.getRoster().size();
					...
					if (rosterSize > 50) {
						...
						// pick the defensive player with the highest
						// athleticism
						// score
						// For the player that is added to the roster - set
						// isInATeam to true !
						// player.setIsInATeam(true);
						// player.setTeam(t);
					} else if (defensiveSize == 25) {
						// pick the offesnive player with the highest
						// athleticism
						// score
						// For the player that is added to the roster - set
						// isInATeam to true !
						...
						// compare the best (highest athleticism score)
						// offensive
						// player
						// to the best defensive player
						// add the one with the higher athleticism score to the
						// team
				...
				it = teamLosstoWinOrder.listIterator();
			}
		}
